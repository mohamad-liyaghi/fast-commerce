...
class TestCartItemDeleteRoute:
    ...
    @pytest.mark.asyncio
    async def test_delete_unauthenticated_fails(self, client) -> None:
        response = await client.delete(self.url)
        assert response.status_code == status.HTTP_403_FORBIDDEN

    @pytest.mark.asyncio
    async def test_delete_existing_item(self, admin_client) -> None:
        response = await admin_client.delete(self.url)
        assert response.status_code == status.HTTP_204_NO_CONTENT

    @pytest.mark.asyncio
    async def test_delete_non_existing_item(self, admin_client) -> None:
        url = f"v1/cart/{uuid4()}"
