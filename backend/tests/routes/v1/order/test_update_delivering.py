...
class TestUpdateToDeliveringRoute:
    ...
    @pytest.mark.asyncio
    async def test_set_delivering_twice_fails(self, admin_client, delivering_order):
    ...
    @pytest.mark.asyncio
    async def test_update_by_other_user_fails(self, authorized_client) -> None:
        response = await authorized_client.put(self.url, json=self.data)
        assert response.status_code == status.HTTP_403_FORBIDDEN

    @pytest.mark.asyncio
    async def test_update_invalid_status_fails(self, admin_client):
        self.data["status"] = str(OrderStatusEnum.PENDING_PAYMENT.value)
        response = await admin_client.put(self.url, json=self.data)
        assert response.status_code == status.HTTP_400_BAD_REQUEST

    @pytest.mark.asyncio
    async def test_update_not_found_fails(self, admin_client):
